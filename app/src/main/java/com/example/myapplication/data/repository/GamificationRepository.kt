 ...
 * streak. A failed day consumes a shield if one exists; otherwise the streak resets.
 * Streak milestone shields are stackable, while the weekly shield is capped at one.
 */
class GamificationRepository(
    ...
    suspend fun sync(): GamificationSyncResult {
        ...
        for ((dateKey, rows) in all) {
            ...
            val successful = rows.isNotEmpty() && rows.all { row ->
                if (row.completed) return@all true
                // A soft 5-minute extension is an intentional user choice and
                // therefore must not break the Streak. It is still limited to
                // the same app/day and only covers the configured grace window.
                protectionPreferences.hasSoftGrace(row.packageName, dateKey) &&
                    row.usedMinutes <= row.limitMinutes + SOFT_GRACE_MINUTES
            }
            if (successful) {
                currentStreak += 1
        ...
        if (lastProcessedDate != null) userPreferences.setLastProcessedStreakDate(lastProcessedDate)
    ...
    fun getAchievements(): List<AchievementDefinition> =
        listOf(
            AchievementDefinition("first_steps", "Первые шаги", "Выберите приложение для отслеживания экранного времени и настройте для него дневной лимит."),
            *STREAK_MILESTONES.map { AchievementDefinition(it.key, it.title, "Серия без превышения лимита: ${it.days} ${dayWord(it.days)}.") }.toTypedArray(),
            *SAVED_TIME_MILESTONES.map { AchievementDefinition(it.key, it.title, "Накоплено ${formatMinutes(it.minutes)} сэкономленного времени.") }.toTypedArray(),
            AchievementDefinition("walks_weekly_challenge", "Маршрут на неделю", "Конвертируйте 3 часа за неделю в прогулки.")
        )

    fun isAchievementUnlocked(key: String): Boolean = userPreferences.isAchievementUnlocked(key)
    ...
    private fun grantWeeklyShieldIfNeeded() {
        ...
        if (userPreferences.getLastWeeklyShieldWeek() != weekKey) {
            ...
            if (userPreferences.getWeeklyShields() == 0) userPreferences.setWeeklyShields(1)
        }
    }
    ...
    companion object {
        private const val SOFT_GRACE_MINUTES = 5
...
data class AchievementDefinition(val key: String, val title: String, val description: String)
