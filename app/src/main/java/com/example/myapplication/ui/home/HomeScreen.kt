    ...
    androidx.compose.foundation.lazy.LazyColumn(
        ...
        item {
            Column(modifier = Modifier.fillMaxWidth(), verticalArrangement = Arrangement.spacedBy(16.dp)) {
                Text(text = if (userName.isBlank()) "Добрый день 👋" else "Добрый день, $userName 👋", style = MaterialTheme.typography.headlineMedium, fontWeight = FontWeight.Bold)
                ...
                if (state.apps.isEmpty()) {
                    ...
                    Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
                        state.apps.forEach { usage ->
                            AppUsageCard(usage.app.appName, usage.usedMinutes, usage.app.dailyLimitMinutes, usage.app.effectiveLimitMinutes)
                        }
                    }
                }
...
private fun SavedTimeCard(todaySavedMinutes: Int, totalSavedMinutes: Int) {
    Card(modifier = Modifier.fillMaxWidth(), shape = RoundedCornerShape(24.dp), colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.secondaryContainer)) {
        Column(modifier = Modifier.padding(20.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
            ...
            Text("Сегодняшнее значение меняется вместе с использованием. В копилку попадает только завершённое время.", style = MaterialTheme.typography.bodySmall)
        }
    }
}
...
private fun SavingsPotCard(state: HomeUiState, onAllocate: (SavingsCategory) -> Unit) {
    Card(modifier = Modifier.fillMaxWidth(), shape = RoundedCornerShape(24.dp), colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.tertiaryContainer)) {
        Column(modifier = Modifier.padding(20.dp), verticalArrangement = Arrangement.spacedBy(14.dp)) {
            ...
            SavingsGoalRow("📚", "Книги", state.booksSavingsMinutes, SavingsCategory.BOOKS.targetMinutes, "≈ ${(state.booksSavingsMinutes / 1.25f).toInt()} страниц", state.availableSavingsMinutes >= 30) { onAllocate(SavingsCategory.BOOKS) }
            SavingsGoalRow("🎬", "Кино", state.moviesSavingsMinutes, SavingsCategory.MOVIES.targetMinutes, "До «Зелёной мили» — ${maxOf(0, SavingsCategory.MOVIES.targetMinutes - state.moviesSavingsMinutes)} мин", state.availableSavingsMinutes >= 30) { onAllocate(SavingsCategory.MOVIES) }
            SavingsGoalRow("🚶", "Прогулки", state.walksSavingsMinutes, SavingsCategory.WALKS.targetMinutes, "≈ ${(state.walksSavingsMinutes / 45f * 3.5f).toInt()} км", state.availableSavingsMinutes >= 30) { onAllocate(SavingsCategory.WALKS) }
            Text("🎯 Задание недели: конвертируй 3 часа в прогулки — ${state.weeklyWalkMinutes}/180 мин", fontWeight = FontWeight.SemiBold)
            LinearProgressIndicator(progress = { (state.weeklyWalkMinutes / 180f).coerceIn(0f, 1f) }, modifier = Modifier.fillMaxWidth())
            if (state.weeklyWalkMinutes >= 180) Text("🏅 Задание выполнено!", fontWeight = FontWeight.Bold)
        }
    }
}

@Composable
private fun SavingsGoalRow(emoji: String, title: String, allocatedMinutes: Int, targetMinutes: Int, description: String, enabled: Boolean, onAllocate: () -> Unit) {
    Column(verticalArrangement = Arrangement.spacedBy(4.dp)) {
        Row(modifier = Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
            Text("$emoji $title", fontWeight = FontWeight.SemiBold, modifier = Modifier.weight(1f))
            Button(onClick = onAllocate, enabled = enabled, modifier = Modifier.size(width = 150.dp, height = 40.dp)) { Text("+30 мин") }
        }
        LinearProgressIndicator(progress = { (allocatedMinutes / targetMinutes.toFloat()).coerceIn(0f, 1f) }, modifier = Modifier.fillMaxWidth())
